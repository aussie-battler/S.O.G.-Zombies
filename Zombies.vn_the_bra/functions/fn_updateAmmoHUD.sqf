...
/* systemChat format ["updateAmmoHud updated"];
...
private _weapon = currentWeapon player;
if (_weapon isEqualTo "") exitWith {
    _ctrl ctrlSetStructuredText parseText "<t size='1.2' color='#FFFFFF' font='tt2020base_vn_bold'>No Weapon</t>";
};

// --- Compatible mags for this weapon ---
private _compatibleMags = getArray (configFile >> "CfgWeapons" >> _weapon >> "magazines");

// --- Player’s mags filtered by compatibility ---
private _mags = magazinesAmmo player;
private _validMags = _mags select { (_x select 0) in _compatibleMags };

// --- Current mag info ---
private _curMag   = currentMagazine player;
private _ammoInMag = player ammo _weapon;

// --- Reserve calculation (exclude exactly one loaded mag) ---
private _reserve = 0;
private _loadedSkipped = false;

{
    ...
    if (!_loadedSkipped && {_magClass == _curMag && _rounds == _ammoInMag}) then {
        _loadedSkipped = true;   // skip loaded mag once
    } else {
        _reserve = _reserve + _rounds;
    };
} forEach _validMags;
...
// --- Format HUD text ---
private _text = format [
    "<t size='3.8' color='#cccccc' font='tt2020base_vn_bold'>%1</t><t size='3' color='#cccccc' font='tt2020base_vn_bold'> / %2</t>",
    _ammoInMag,
    _reserve
];
